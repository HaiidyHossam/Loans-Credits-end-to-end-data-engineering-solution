...
df_raw = spark.readStream \
    ...
    .option("kafka.group.id", 'fact_consumer') \
...
df_transformed_fact= df_parsed.select(fact_columns)



final_columns_of_FACT_TABLE = [
    'LOAN_AMOUNT', 'FUNDED_AMOUNT_INVESTOR', 'INTEREST_RATE', 'INSTALLMENT',
 'LOAN_STATUS', 'DTI', 'REVOLVING_BALANCE', 'REVOLVING_UTILIZATION',
 'TOTAL_CURRENT_BALANCE', 'TOTAL_BALANCE_INSTALLMENT', 'MAXIMUM_BALANCE',
 'DELINQUENT_AMOUNT', 'ISSUE_DATE', 'LOANPRODUCT_BK']

# Rename columns in the DataFrame according to the column mapping
for source_col, dest_col in column_mapping_fact.items():
    if source_col in df_transformed_fact.columns:
        df_transformed_fact = df_transformed_fact.withColumnRenamed(source_col, dest_col)

df_transformed_fact = df_transformed_fact.select(final_columns_of_FACT_TABLE)
...
def write_to_snowflake(batch_df, batch_id):
    ...
    batch_df.write \
        .format("snowflake") \
        .options(**sfOptions) \
        .option("dbtable", "FACT_STREAMING_STAGE") \
        .mode("append") \
        .save()
...
query = df_transformed_fact.writeStream \
    .outputMode("append") \
