...
df_raw = spark.readStream \
    ...
    .option("subscribe", "hardship_staging") \
    .option("kafka.group.id", 'hardship_consumer') \
...
DIM_HARDSHIP_columns = ["hardship_flag", "hardship_type", "hardship_reason", 
                        "hardship_status","hardship_amount", "hardship_start_date", "hardship_end_date", "hardship_length","hardship_dpd",
                     "hardship_loan_status","hardship_payoff_balance_amount","id"]

df_transformed_hardship =df_parsed.filter(df_parsed.hardship_flag == 'Y').select(DIM_HARDSHIP_columns)
#filter(df_parsed.hardship_flag == 'y').

        

final_HARDSHIP_COLUMNS = ["HARDSHIP_FLAG", "HARDSHIP_TYPE", "HARDSHIP_REASON", "HARDSHIP_STATUS","HARDSHIP_AMOUNT", "HARDSHIP_START_DATE", "HARDSHIP_END_DATE", "HARDSHIP_LENGTH"
    ,"HARDSHIP_DPD",
...
for source_col, dest_col in column_mapping_Hardship.items():
    if source_col in df_transformed_hardship.columns:
        df_transformed_hardship = df_transformed_hardship.withColumnRenamed(source_col, dest_col)

df_transformed_hardship = df_transformed_hardship.select(final_HARDSHIP_COLUMNS)
...
df_transformed_hardship = df_transformed_hardship.withColumn(
    "HARDSHIP_ID_PK",
    F.abs(F.hash(F.concat_ws("|", *df_transformed_hardship.columns)))  
)

final_HARDSHIP_COLUMNS = [ "HARDSHIP_ID_PK","HARDSHIP_FLAG", "HARDSHIP_TYPE", "HARDSHIP_REASON", "HARDSHIP_STATUS","HARDSHIP_AMOUNT", "HARDSHIP_START_DATE", "HARDSHIP_END_DATE", "HARDSHIP_LENGTH"
    ,"HARDSHIP_DPD",
    "HARDSHIP_LOAN_STATUS","HARDSHIP_PAYOFF_BALANCE_AMOUNT","LOANPRODUCT_BK"] 


df_dim_hardship_finaaal = df_transformed_hardship.select(final_HARDSHIP_COLUMNS)
#####################################################################################################################
...
query = df_dim_hardship_finaaal.writeStream \
    .outputMode("append") \
